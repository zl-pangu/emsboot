...
(function (factory) {
    ...
    FileInput.prototype = {
        ...
        _initPreviewActions: function () {
            ...
            self._initZoomButton();
            var beforeDelFunc = function ($el) {
                layer.confirm('是否删除文件？',{
                    btn:['是','否']
                },function (index) {
                    layer.close(index);
                    $el();
            ...
            $preview.find(btnRemove).each(function () {
                ...
                var $frame = $el.closest($h.FRAMES), cache = self.previewCache.data,
                    settings, params, index = $frame.attr('data-fileindex'), config, extraData,beforeDel;
                index = parseInt(index.replace('init_', ''));
                config = $h.isEmpty(cache.config) && $h.isEmpty(cache.config[index]) ? null : cache.config[index];
                extraData = $h.isEmpty(config) || $h.isEmpty(config.extra) ? deleteExtraData : config.extra;
                beforeDel = config.beforeDel;
        ...
        _readFiles: function (files) {
            ...
            readFile = function (i) {
                ...
                var node = ctr + i, previewId = previewInitId + "-" + node, file = files[i], fSizeKB, j, msg,
                    ...
                    caption = file.name ? self.slug(file.name) : '', fileSize = (file.size || 0) / 1024,
